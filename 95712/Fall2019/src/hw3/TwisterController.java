 ...
 * @andrewID: qiuchenz
 * 
 * @function: Controller methods of the game twister.
 */
...
public class TwisterController extends WordNerdController{
	...
	/**
	 *  Creates new Twister, TwisterView, invokes setupRound() to create a new TwisterRound,
	...
	/**
	 * Binds GUI components with TwisterRound properties and the timer
	...
	void setupBindings() {
		...
		twister.twisterRound.isRoundCompleteProperty().addListener((observable, oldValue, newValue) -> {
			if (newValue) {
				...
				float score = (float)twister.submittedCount() / twister.solutionCount();
				// scoreString format: "gameId,puzzleWord,time,score"
				String scoreString = String.format("%d,%s,%d,%f", 1, twister.twisterRound.getPuzzleWord(), eclipseTime, score);
	...
	/** 
	 * Starts a new round of game, refresh the view,
	...
	/** 
	 * Takes chars in ansButton as a guess, submits the guess to Twister.nextTry(), 
	...
	/** 
	 * Takes the char in clicked answerButton, finds the first empty clueButton, 
	...
	/** 
	 * Takes the char in clicked clueButton, finds the first empty ansButton, 
	 ...
	 * 
	 * @param srcButton: button from which move value out of
	 * @param dstButtons: button list in which to find an empty button
	 */
